...
board::board(int sqSize)
    ...
    rows(max_len, vector<bool>(max_len, 0)),
    cols(max_len, vector<bool>(max_len, 0)),
    sqs(max_len,  vector<bool>(max_len, 0))
    // Board constructor
{
}
    ...
    for (int i = 1; i <= BoardSize; i++)
        for (int j = 1; j <= BoardSize; j++) {
            ...
            if (ch != '.') {
                ...
                rows[i].at(_atoi(ch)) = true;
                cols[j].at(_atoi(ch)) = true;
                sqs[squareNumber(i,j)].at(_atoi(ch)) = true;
            }
        }
    }
}
